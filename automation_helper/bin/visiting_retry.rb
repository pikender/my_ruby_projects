...
list_of_ips.each do |l|
  no_extensions = true
  found_file = csv_print_reports_downloaded.grep(/#{l['ip']}/)
  if found_file.any?
    find_extnames = found_file.select {|k| File.extname(k) == '.crdownload'}
    no_extensions = (found_file.size == find_extnames.size)
  else
    no_extensions = true
  end
  if no_extensions
