...
begin
  ...
  post_ppi_logger.info("Printer Reports Downloaded Files:: #{csv_print_reports_downloaded.join(', ')}")
  ...
  if csv_print_reports_downloaded.any?
    ...
    log_files.each_with_index do |l, i|
      post_ppi_logger.info("Report Log File #{i}: #{l}")
    ...
    smtp_info = YAML.load_file(current_path.join('config', 'smtp_info.yml').to_s)
    ...
    post_ppi_logger.info('Mail Sent with Zipped Report Contents')
    # Upload on WeConnect
