...
debug_file_path = current_path.join('log', "debug#{f_todays_date}.log").to_s
debug_file = File.open(debug_file_path, 'a')

## Create a Ruby logger
debug_logger = Logger.new(Wework::MultiIO.new(STDOUT, debug_file))
debug_logger.level = Logger::DEBUG
debug_logger.progname = ENV['PRINTER_URL']
debug_logger.datetime_format = "%Y-%m-%d %H:%M:%S"
